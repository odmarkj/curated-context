...
function getPositionalArg(minIndex: number): string | undefined {
...
function showMemories(isGlobal: boolean) {
...
function forgetMemory(key: string | undefined, isGlobal: boolean) {
  if (!key) {
...
function promoteMemory(key: string | undefined) {
  if (!key) {
...
function searchMemories(query: string) {
  ...
  if (projectMatches.length > 0) {
    ...
    const grouped: Record<string, typeof projectMatches> = {};
  ...
  if (globalMatches.length > 0) {
    ...
    const grouped: Record<string, typeof globalMatches> = {};
