 ...
 */
export function computeEffectiveConfidence(mem: StoredMemory, now?: number): number {
  if (mem.protected) return mem.confidence;

  const currentTime = now ?? Date.now();
  const msSinceAccess = currentTime - (mem.lastAccessed || mem.updatedAt);

  if (msSinceAccess <= 0) return mem.confidence;
 ...
 */
export function touchMemory(mem: StoredMemory): void {
 ...
 */
export function isDecisionMemory(value: string): boolean {
 ...
 */
export function autoProtect(mem: StoredMemory): void {
...
export interface StoredMemory {
  ...
  sessionId: string;
  protected?: boolean;
  // Phase 1.5.5: Topic Key Upsert
  topicKey?: string;
  revisionCount?: number;
  // Phase 1.5.6: Content Hash Dedup
  contentHash?: string;
  duplicateCount?: number;
  // Phase 1.5.7: Contradiction Tracking
 ...
 */
export function inferTopicKey(category: string, key: string): string | undefined {
  // Normalize: lowercase, replace separators with /
  const normalized = key.toLowerCase().replace(/[-_\s]+/g, '-');
  const words = normalized.split('-').filter(Boolean);

  // If all words are hub terms, skip topic key assignment
  if (words.every((w) => HUB_TERMS.has(w))) return undefined;
...
function jsonStorePath(projectRoot: string): string {
 ...
 */
function migrateJsonToSqlite(projectRoot: string, db: Database.Database): void {
  const jsonPath = jsonStorePath(projectRoot);
  if (!existsSync(jsonPath)) return;

  // Check if DB already has data (avoid double-migration)
  const count = db.prepare('SELECT COUNT(*) as cnt FROM memories').get() as any;
  if (count.cnt > 0) return;

  try {
    const store: MemoryStore = JSON.parse(readFileSync(jsonPath, 'utf8'));
    ...
    const tx = db.transaction(() => {
      for (const mem of Object.values(store.memories)) {
        ...
        if (!mem.protected && isDecisionMemory(mem.value)) isProtected = 1;
        ...
        insert.run(
          ...
          mem.source || null, mem.filePattern || null,
          mem.createdAt, mem.updatedAt, lastAccessed,
          mem.sessionId, isProtected,
          topicKey || null, 1, contentHash, 1,
          'active', null, null,
          0, 1, 0.5,
        );
      }
...
export function loadStore(projectRoot: string): MemoryStore {
  ...
  const memories: Record<string, StoredMemory> = {};
  ...
  const lastConsolidated = Number((getMeta.get('last_consolidated') as any)?.value ?? 0);
  const lastUpdated = Number((getMeta.get('last_updated') as any)?.value ?? 0);
...
export function saveStore(projectRoot: string, store: MemoryStore): void {
  ...
  if (entries.length > maxEntries) {
    entries.sort((a, b) => {
      if (a[1].protected && !b[1].protected) return 1;
      if (!a[1].protected && b[1].protected) return -1;
  ...
  const tx = db.transaction(() => {
    ...
    for (const mem of Object.values(store.memories)) {
      insert.run(
        mem.key, mem.category, mem.value, mem.confidence,
        mem.source || null, mem.filePattern || null,
        mem.createdAt, mem.updatedAt, mem.lastAccessed || mem.updatedAt,
        mem.sessionId, mem.protected ? 1 : 0,
        mem.topicKey || null, mem.revisionCount || 1,
        mem.contentHash || null, mem.duplicateCount || 1,
        mem.status || 'active', mem.supersededBy || null, mem.contradictedBy || null,
        mem.verified ? 1 : 0, mem.observedSessions || 1, mem.helpfulnessEma ?? 0.5,
      );
    }
...
export function getMemoriesByCategory(
  ...
  for (const mem of Object.values(store.memories)) {
    ...
    if (mem.status && mem.status !== 'active') continue;
 ...
 */
export function searchMemories(projectRoot: string, query: string, limit = 20): StoredMemory[] {
  const dir = storeDir();
  mkdirSync(dir, { recursive: true });
...
export function recallMemories(
  projectRoot: string,
  query: string,
  options?: { topic?: string; maxTokens?: number; limit?: number },
): { memories: StoredMemory[]; formatted: string } {
  ...
  const scored = rows.map((row: any) => {
    ...
    let topicBoost = 1.0;
    if (topic && mem.category === topic) topicBoost = 1.3;
    if (topic && mem.topicKey?.startsWith(topic + '/')) topicBoost = 1.3;
  ...
  for (const { mem } of scored) {
    ...
    if (tokenEstimate + memTokens > maxTokens && memories.length > 0) break;
    memories.push(mem);
