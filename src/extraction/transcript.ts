 ...
 */
export function stripPrivateContent(text: string): { sanitized: string; strippedCount: number } {
  let strippedCount = 0;
  let result = text;
  ...
  for (const block of blocks) {
    if (block.type === 'text' && block.text) {
      ...
      if (typeof content === 'string') {
        ...
        parts.push(extractTextFromBlocks(content as ContentBlock[]));
      }
    }
  }
  return parts.join('\n');
...
export function computeTranscriptHash(messages: ConversationMessage[]): string {
