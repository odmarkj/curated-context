...
function buildDefaultContent(category, memories, sizeLimit) {
...
function buildPreferencesContent(memories, sizeLimit) {
...
function buildDataContent(memories, sizeLimit) {
    ...
    const other = memories.filter((m) => !m.key.startsWith('data-file-') && !m.key.startsWith('canonical-') &&
    ...
    for (const section of sections) {
        ...
        if (Buffer.byteLength(content + headingLine, 'utf8') > maxSize)
            break;
        content += headingLine;
        for (const mem of section.items) {
            const line = `- **${mem.key}**: ${mem.value}\n`;
            if (Buffer.byteLength(content + line, 'utf8') > maxSize)
                break;
            content += line;
        }
        content += '\n';
