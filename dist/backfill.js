...
import { processSessionCore } from './daemon/processor.js';
 ...
 */
export function projectToSlug(projectPath) {
 ...
 */
export function discoverClaudeMemSessions(projectPath) {
...
export async function runBackfill(options) {
    ...
    for (let i = 0; i < sessions.length; i++) {
        ...
        try {
            ...
            if (verbose) {
                const parts = [];
                if (stats.memoriesFromStructural > 0)
                    parts.push(`${stats.memoriesFromStructural} structural`);
                if (stats.memoriesFromApi > 0)
                    parts.push(`${stats.memoriesFromApi} api`);
                if (stats.apiCallsMade > 0)
                    parts.push(`${stats.apiCallsMade} api calls`);
