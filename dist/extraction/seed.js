 ...
 */
export function seedProject(projectRoot) {
    const memories = [];
    let filesScanned = 0;
    // --- package.json ---
    ...
    if (existsSync(pyprojectPath)) {
        ...
        try {
            ...
            if (content.includes('fastapi'))
                memories.push({ category: 'architecture', key: 'dep-fastapi', value: 'Uses FastAPI', confidence: 0.6, source: 'seed' });
            if (content.includes('django'))
                memories.push({ category: 'architecture', key: 'dep-django', value: 'Uses Django', confidence: 0.6, source: 'seed' });
            if (content.includes('flask'))
                memories.push({ category: 'architecture', key: 'dep-flask', value: 'Uses Flask', confidence: 0.6, source: 'seed' });
        }
        catch { /* */ }
    }
    // --- Cargo.toml ---
